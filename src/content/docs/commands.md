...
| [OpenAI Codex](/ides/openai-codex) | `.agents/skills/` (skills only) | `$skill-name` | Skills-only (no slash commands) | [Full setup →](/ides/openai-codex#commands) |

**Note:** OpenAI Codex does not support slash-command markdown files — its reusable-instruction mechanism is the Agent Skills standard (`$skill-name` invocation from `.agents/skills/`). To share commands with a Codex user, author them as skills. Windsurf and Kilo Code call their commands "Workflows" — this is their storage mechanism for reusable instructions, not factory engineering workflows. GitHub Copilot uses `.prompt.md` files with different naming and optional frontmatter, so commands require a sync step rather than a symlink. See [GitHub Copilot](/ides/github-copilot#commands) for details.
